...
""" V1 - 02/07/2019 - this program presents user with a static window containing the game layout; spawns a single
                                      alien for a specified number of times, which moves towards the earth and disappears upon collision.
"""
# IMPORTS
...
def move_alien(init_coords, alien_id, speed):
    ...
    if spawn in ('spawn1', 'spawn2', 'spawn8'):
        while game_canvas.coords(widget_alien) != [x_ea_c,y_ea_c]:
            if  x_pos < x_ea_c :
                game_canvas.move(widget_alien,1,0)
...
root = tk.Tk()
root.title("Keyboard_Warrior_v1")
...
image_space_background = tk.PhotoImage(file="images/static/image_space_background.png")
game_background = game_canvas.create_image(0, 0, image = image_space_background, anchor = 'c')

# Create earth widget
image_earth = tk.PhotoImage(file="images/static/image_earth.png")
widget_earth = game_canvas.create_image(350,300, image = image_earth, anchor = "c")

# Create sun widget
image_sun = tk.PhotoImage(file = "images/static/image_sun.png")
